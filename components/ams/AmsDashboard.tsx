...
function BiographyPanel({
  ...
  const selectedExtra = biographyExtraForPlayer(selectedPlayer);
  const selectedStats = biographyStatsForPlayer(selectedPlayer);
    ...
    <div className="biography-panel">
      ...
      <section className="biography-deep-grid">
        <article className="biography-deep-panel">
          <div className="biography-deep-heading">
            ...
            <strong>{language === "es" ? "Resumen de jugador" : "Player summary"}</strong>
          </div>
      ...
      <section className="biography-table-panel">
        <div className="biography-deep-heading">
          ...
          <strong>{language === "es" ? "Pendiente de API oficial" : "Official API pending"}</strong>
        </div>
        <p>
          {language === "es"
...
function biographyExtraForPlayer(player: Player) {
  ...
  return extras[player.id] ?? {
    birthDate: player.age === "Pending" ? "Pending API" : "Pending source merge",
    birthPlace: "Pending API",
    previousClub: "Pending API",
    joined: "Pending API",
    contractExpires: "Pending API",
    marketValue: "Pending API",
    contractProgress: player.status === "synced" ? 46 : player.status === "review" ? 30 : 14,
  };
}

function biographyStatsForPlayer(player: Player) {
  const stats: Record<string, {
    ...
    "camilo-vargas": { appearances: 18, starts: 18, minutes: 1620, goals: 0, assists: 0, rating: "6.6" },
    "gustavo-ferrareis": { appearances: 16, starts: 15, minutes: 1303, goals: 0, assists: 0, rating: "Pending" },
    "aldo-rocha": { appearances: 17, starts: 17, minutes: 1488, goals: 1, assists: 2, rating: "6.9" },
    "mateo-garcia": { appearances: 15, starts: 12, minutes: 1034, goals: 2, assists: 3, rating: "6.8" },
    "manuel-capasso": { appearances: 14, starts: 13, minutes: 1160, goals: 1, assists: 0, rating: "6.7" },
  };

  if (stats[player.id]) return stats[player.id];
  if (player.status === "synced") return { appearances: 12, starts: 9, minutes: 824, goals: 0, assists: 1, rating: "Pending" };
  if (player.status === "review") return { appearances: 8, starts: 6, minutes: 540, goals: 0, assists: 0, rating: "Review" };
  return { appearances: 0, starts: 0, minutes: 0, goals: 0, assists: 0, rating: "Pending API" };
}
